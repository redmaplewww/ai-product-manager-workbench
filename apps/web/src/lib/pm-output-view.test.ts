...
describe("readPmProposal", () => {
  it("reads the PM proposal from a persisted run output", () => {
    expect(readPmProposal({ answer: "已回答", reviewResponses: [], proposalTitle: "HR 初筛", proposalRationale: "整理业务场景", proposalItems: [{ itemIds: ["a"], category: "audience", content: "HR 招聘人员", selected: true }] })).toEqual({
      title: "HR 初筛",
      rationale: "整理业务场景",
  ...
  it("ignores malformed or missing PM output", () => {
    ...
    expect(readPmProposal({ proposal: [{ category: "audience", content: "旧格式" }] })).toBeUndefined();
