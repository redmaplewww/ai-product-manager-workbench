...
export function LoginForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(apiUrl("/api/auth/login"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ username: data.get("username"), password: data.get("password") }) });
    const body = await response.json();
    if (!response.ok) { setError(body.error || "登录失败"); setBusy(false); return; }
    location.href = `${BASE_PATH}${body.mustChangePassword ? "/change-password" : "/"}`;
  }
  return <section className="login-panel">
    ...
    <form onSubmit={submit} className="form-stack" method="post" action={apiUrl("/api/auth/login")}>
