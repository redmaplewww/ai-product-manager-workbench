...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const url = await assertSafePublicUrl(String(body.url));
      ...
      source = { kind: "web", title: String(body.title || url.hostname), url: url.toString(), excerpt: text.slice(0, 1200), size: Buffer.byteLength(html) };
    }
